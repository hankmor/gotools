...
func TestEncode(t *testing.T) {
	...
	for _, test := range tt {
		got := base64s.Base64UrlUint.Encode(test.input())
...
func Test_Base64UrlUintDecode(t *testing.T) {
	...
	for _, test := range tt {
		got, err := base64s.Base64UrlUint.Decode(test.input)
		if test.valid {
...
func TestEncLongString(t *testing.T) {
	bi, err := base64s.Base64UrlUint.Decode(s2)
	if err != nil {
	...
	if !ok {
		...
		enc := base64s.Base64UrlUint.Encode(it)
		if enc != s2 {
			t.Errorf("result is incorrect")
			t.Failed()
		}
